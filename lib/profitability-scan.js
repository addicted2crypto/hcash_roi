...
// ─── Persistence ──────────────────────────────────────────────────────────
// Uses lib/storage.js — Blob in production, fs in dev/CLI scripts.

export async function loadCheckpoint() {
  return await getJson(KEYS.SCAN_CHECKPOINT, null);
}

async function saveCheckpoint(state) {
  await putJson(KEYS.SCAN_CHECKPOINT, state);
}

async function saveOutputs(cohorts, wallets) {
  await putJson(KEYS.COHORTS, cohorts);
  await putJson(KEYS.WALLET_PNL, wallets);
}
...
export async function runScan({
  ...
  await saveOutputs(cohorts, walletIndex);
