...
// ABI cache uses bundled fs only — these don't change at runtime and the
// bundled files ship with the deploy. Read-only filesystem is fine.
async function loadAbi(abiId) {
  ...
  // Best-effort write to /tmp-style path; if read-only, just return the data
  try {
    fs.mkdirSync(path.dirname(cache), { recursive: true });
    fs.writeFileSync(cache, JSON.stringify(json.abi, null, 2));
  } catch { /* read-only fs in prod — fine, just don't cache */ }
  return json.abi;
}
...
export async function runWatcher() {
  ...
  const state = await getJson(KEYS.WATCHER_STATE, { lastBlock: null, cadenceMin: 5, quietRuns: 0 });
  const prevDelta = await getJson(KEYS.LEADERBOARD_DELTA, null);
  const prevCostsFile = await getJson(KEYS.COST_CHANGES, null);
  const prevCosts = prevCostsFile?.lastSnapshot || null;
  const costLog   = prevCostsFile?.changes || [];
  ...
  await putJson(KEYS.LAUNCHING, { updatedAt: new Date().toISOString(), launching });
  ...
  await putJson(KEYS.LEADERBOARD_DELTA, delta);
  ...
  await putJson(KEYS.COST_CHANGES, { updatedAt: new Date().toISOString(), lastSnapshot: nextCosts, changes: mergedLog });
  ...
  await putJson(KEYS.WATCHER_STATE, {
...
export async function loadWatcherState() { return await getJson(KEYS.WATCHER_STATE, null); }
export async function loadDelta()        { return await getJson(KEYS.LEADERBOARD_DELTA, null); }
export async function loadCostChanges()  { return await getJson(KEYS.COST_CHANGES, { changes: [] }); }
export async function loadLaunching()    { return await getJson(KEYS.LAUNCHING, { launching: [] }); }
