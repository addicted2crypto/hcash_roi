...
export async function getJson(name, fallback = null) {
  ...
  let data;
  if (USE_BLOB) {
    data = await blobRead(name);
    // Fallback: if Blob returns null (404, capped, forbidden), try the bundled fs
    // copy that ships with the deploy. This keeps the dashboard functional during
    // Hobby tier ops caps (writes blocked until reset, reads can come from build).
    if (data === null) data = fsRead(name);
  } else {
    data = fsRead(name);
  }
...
export async function putJson(name, data) {
  ...
  if (USE_BLOB) {
    try {
      ...
      // Blob write failed (likely capped). Don't surface — cache layer holds the
      // value for this process lifetime, and bundled fs serves the previous deploy's data.
      // Errors visible in Vercel logs but don't crash the cron route.
      console.warn(`[storage] Blob write failed for ${name}: ${err?.message || err}`);
    }
  } else {
    fsWrite(name, data);
  }
...
export async function statJson(name) {
  if (USE_BLOB) {
    const blobS = await blobStat(name);
    if (blobS) return blobS;
    // Fallback to bundled fs stat
    const s = fsStat(name);
