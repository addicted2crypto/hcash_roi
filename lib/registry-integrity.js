...
//
// Output: KEYS.INTEGRITY_ISSUES (Blob in prod, fs in dev) — consumed by
// /api/game and the UI banner.
...
async function loadIssues() {
  const raw = await getJson(KEYS.INTEGRITY_ISSUES, null);
  return raw?.issues || [];
}

async function saveIssues(issues) {
  await putJson(KEYS.INTEGRITY_ISSUES, {
...
export async function runIntegrityCheck(contracts) {
  const issues = validateRegistry(contracts);
  await saveIssues(issues);
  return { issues, count: issues.length, byKind: tally(issues) };
}

export async function loadIntegrityIssues() {
  return await loadIssues();
}
